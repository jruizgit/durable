...
function baseVisual(parent, history) {
	...
	var updateAxis = function(newStartTime, newEndTime) {
		...
		timeLabel.text(selectedTime.toLocaleString());
		axis.call(yAxis);
...
function promiseHistory(url) {
	...
	var errorEvents = [];
	var selectedRecord;
	var promiseUrl = url.substring(0, url.indexOf("/", 1));				
...
function promiseGraph(url, history) {
	...
	var getStateNodes = function(chart, links) {    	
		var currentState;
		...
		for (var stateName in chart) {
			...
			if (currentState.$chart) {			
				stateNode = getStateNodes(currentState.$chart, links);
				stateNode.id = stateName;
			}
			else {
				stateNode = { id: stateName, history: [], size:20 }; 
			}
		...
		for (var stateName in chart) {		
			...
			for (transitionName in currentState) {			
				...
				var transitionNode = { r:14, name: stateName, type:"tag", top : true, children: [] };
				var transitionLinks = [];
				var outputNode;
