exports = module.exports = durable = function () {
    ...
    var session = function (document, messages, sessions, db) {
                    ...
                    if (that.receivedMessages[result.id]) {
                        ...
                        messages.find(query, { fields: { _id: 0 } }).nextObject(function (err, result) {
                            if (err) {
                                ...
                                that.receivedMessages[result.id] = { count: 0 };
        ...
        that.rollbackReceive = function (messageId) {
            var retry = that.receivedMessages[messageId];
            if (retry && !retry.count) {
                delete that.receivedMessages[messageId];
            }
        }
                                ...
                                if (err) {
                                    ...
                                    sessions.update({ _id: oldId }, { $set: { status: "checked" }, $unset: { tag: "" } }, function (err, result) {
                                        ...
                                        for (var messageId in that.receivedMessages) {
                                            messagesQuery.push({ id: messageId, session: that.id });
                                        }
    ...
    var tryEvents = function (events, all) {
        ...
        var resolve = function (eventName, events, rollbackEvents) {
            ...
            if (eventName.indexOf("$and") !== -1) {
                ...
                if (rollbackEvents && event.rollback) {
        ...
        var noFunctionWithRollback = function (rollbackEvents) {
            var events = [];
            for (var i = 0; i < rollbackEvents.length; ++i) {
                events.push(rollbackEvents[i]);
            }

            var noFunction = function (session) {
                for (var i = 0; i < events.length; ++i) {
                    events[i].rollback(session);
                }
        ...
        var and = function (events) {
            ...
            for (var andEventName in events) {
                if (!currentAndPromise) {
                    currentAndPromise = promise(firstFunction).continueWith(branch(resolve(andEventName, events, rollbackEvents)));
                }
                else {
                    currentAndPromise = currentAndPromise.continueWith(branch(resolve(andEventName, events, rollbackEvents)));
                }
                ...
                currentAndPromise.elseContinue(noFunctionWithRollback(rollbackEvents)).setNext(that);
    ...
    var stateChart = function (chart, parentStateName, parentStateRoot) {
        ...
        var validateTransition = function (transition) {
            for (var keyword in transition) {
                if (keyword !== "when" &&
					keyword !== "whenAll" &&
					keyword !== "whenAny" &&
					keyword !== "run" &&
					keyword !== "to")
                    throw keyword + " is not a valid transition directive.";
            }
        }
        ...
        var createTransitions = function (state, transitions) {
            ...
            for (transitionName in state) {
                if (transitionName != "$chart") {
                    ...
                    validateTransition(currentTransition);
                    if (!rootStatePromise) {
    ...
    var tryReceive = function (query, projection) {
        ...
        var rollback = function (session) {
            if (messageId) {
                session.rollbackReceive(messageId);
            }
        }
        ...
        that.thenCondition = function (func) {
            return that.continueWith(tryCondition(function (s, context) {
                if (s.getOutput().checked) {
                    if (func(s)) {
                        return true;
                    }
                    context.rollback(s);
    ...
    var tryTimer = function (timerName) {
        ...
        var that = promise(function (session) {
            session.setInput({ query: { effective: { $lt: new Date().getTime() }, name: timerName } });
        })
